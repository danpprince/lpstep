...
# Create class variables for sending notes generated by the sequencers
# in a separate thread to prevent timing errors
note_queue = Queue()
def send_note(queue):
    while True:
    ...
    def stop_note(self):
        ...
        if step_state and not self.muted:
            ...
            if self in get_current_seq_page():
                self.view.update(step, step_state)
            note_queue.put([144, self.note_num, 0])
    ...
    def start_note(self, step):
        ...
        if step_state and not self.muted:
            ...
            if step_state == notestates.NOTE_VEL_HIGH:
                note_queue.put([144, self.note_num, 127])
            elif step_state == notestates.NOTE_VEL_LOW:
                note_queue.put([144, self.note_num, 60])
        else:
            # Update the view if this sequencer is in the displayed page
            if self in get_current_seq_page():
                self.view.update(step, notestates.NOTE_PLAYING)
...
sequencer_models = [[SequencerModel([0, 3], 32, 38),
                     SequencerModel([4, 5], 16, 39),
                     SequencerModel(   [6],  8, 41),
                     SequencerModel(   [7],  8, 42)],

                    [SequencerModel([0, 1], 16, 43),
                     SequencerModel([2, 3], 16, 44),
                     SequencerModel([4, 5], 16, 45),
                     SequencerModel([6, 7], 16, 46)]]
